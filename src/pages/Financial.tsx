...
export default function Financial() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(ordersRef, (snapshot) => {
      ...
      const fetchCustomerNames = async (orders: any[]) => {
        const customerIds = [...new Set(orders.map(o => o.customerId).filter(Boolean))];
        const customerMap: Record<string, string> = {};
        
        for (const id of customerIds) {
          try {
            const customerDoc = await getDocs(query(collection(db, `tenants/${tenantId}/customers`), where('__name__', '==', id)));
            if (!customerDoc.empty) {
              customerMap[id] = customerDoc.docs[0].data().name;
            }
          } catch (err) {
      ...
      const processOrders = async () => {
        ...
        const customerMap = await fetchCustomerNames(ordersData);

        ordersData.forEach(data => {
          const amount = Number(data.totalAmount) || 0;
          const customerName = data.customerId ? customerMap[data.customerId] : 'Cliente não informado';
          ...
          if (data.status === 'fechada') {
            ...
            txs.push({
              id: data.id,
              description: `OS #${data.id.slice(0, 8)}`,
              customer_name: customerName || 'Cliente não informado',
              total_amount: amount,
              type: 'income',
              date: data.createdAt,
              status: 'Finalizada'
            ...
            txs.push({
              id: data.id,
              description: `OS #${data.id.slice(0, 8)} (Pendente)`,
              customer_name: customerName || 'Cliente não informado',
              total_amount: amount,
              type: 'income',
              date: data.createdAt,
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="inline-flex items-center px-4 py-2 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-gray-800 transition-all shadow-md">
